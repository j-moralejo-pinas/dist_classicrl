...
class TestParallelQLearning:
    ...
    def setup_method(self):
        ...
        with patch("multiprocessing.shared_memory.SharedMemory"):
            with patch("multiprocessing.Lock"):
                with patch("multiprocessing.Value"):
    ...
    def test_train_with_multiple_environments(self, mock_queue, mock_pipe, mock_process):
        ...
        with patch.object(self.agent, "evaluate_steps", return_value=(5.0, [2.0, 3.0])):
            # Mock shared memory cleanup to avoid issues
    ...
    def test_run_steps_simple_env(self, mock_shared_memory):
    ...
    @patch("multiprocessing.shared_memory.SharedMemory")
    def test_run_steps_dict_observation_env(self, mock_shared_memory):
    ...
    @patch("multiprocessing.Pipe")
    @patch("multiprocessing.Queue")
    def test_reward_collection_during_training(self, mock_queue, mock_pipe, mock_process):
    ...
    def test_shared_memory_cleanup(self):
        ...
        with patch.object(self.agent, "sm") as mock_sm:
            ...
            with patch("multiprocessing.Process"):
                with patch("multiprocessing.Pipe"):
